...
@interface TPPushRecommendDemandViewController()<UITextViewDelegate,UIAlertViewDelegate>


@end


@implementation TPPushRecommendDemandViewController{
...
-(void)setDemandDict:(NSDictionary *)dict{
    data = dict;
...
-(void)rightBtnPress{//发布
    ...
    [dict setObject:str forKey:@"require.service"];
    [dict setObject:[[data objectForKeyNotNull:@"city_code"] stringValue] forKey:@"require.codes"];

    
    
    [self showLoadingHUDWithText:nil];
    __block TPPushRecommendDemandViewController *blkSelf = self;
...
- (BOOL)judgeParameter{
    ...
    if (children.account == 0) {
        [self showHUDWithText:@"请选择儿童数量"];
        result = NO;
        return result;
    }
...
-(void)setServiceUI{
    ...
    [carBtn setImage:carService  withTitle:@"带车" forState:UIControlStateNormal];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    UIViewController * vc = (UIViewController *)vcs[vcs.count-3];
