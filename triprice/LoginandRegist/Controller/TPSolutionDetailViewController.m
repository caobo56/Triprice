//
//  TPSolutionDetailViewController.m
//  triprice
//
//  Created by caobo56 on 16/3/17.
//
//
...
@interface TPSolutionDetailViewController ()<UITableViewDataSource,UITableViewDelegate>

@end

@implementation TPSolutionDetailViewController{
...
-(void)setSolutionID:(NSString *)dict{
    freeTavelingID = dict;
}
...
-(void)searchSpecialById{
    ...
    __block TPSolutionDetailViewController *blkSelf = self;
    [[JDDAPIs sharedJDDAPIs]searchSolutionByIdWithParameters:dic WithBlock:^(NSDictionary *dict, NSString *error) {
        [blkSelf hideAllHUD];
        if (dict) {
...
-(void)searchScheduleBySpecialId{
    ...
    [dic setValue:freeTavelingID forKey:@"id"];
    __block TPSolutionDetailViewController *blkSelf = self;
    [[JDDAPIs sharedJDDAPIs]searchSolutionBySolutionIdWithParameters:dic WithBlock:^(NSDictionary *dict, NSString *error) {
        [blkSelf hideAllHUD];
        if (dict) {
